...
- `change` **Canonical des Logical Models geändert.** Er lautete `…/StructureDefinition/LogicalModel/Seltene` und lautet jetzt `…/StructureDefinition/LogicalModel/mii-lm-seltene`. Das Segment `LogicalModel/` bleibt — es ist MII-weit die Mehrheit —, nur das Ende trägt statt des Namens die Id, wie der IG Publisher es verlangt. **Verweise auf den alten Canonical lösen nicht mehr auf.** Die 17 Mapping-Ziele im Modul sind mitgezogen. Zur Einordnung: Das alte Muster ist MII-weit die Mehrheit — 15 von 23 Logical Models in 10 Modulen führen `…/StructureDefinition/LogicalModel/<Name>`, `kerndatensatz-base` eingeschlossen; sieben Module nutzen bereits das Id-Schema. Der Canonical des CapabilityStatements (`…/CapabilityStatement/metadata`) bleibt unverändert und ist per `special-url` von der Prüfung ausgenommen
- `change` **Benennung durchgehend an die Modulkonvention angeglichen.** Die Prüfregeln in `qc/custom.rules.yaml` verlangen `mii-<typ>-seltene-…` für Ids und `MII_<TYP>_Seltene_…` für Namen; beides war nur teilweise umgesetzt. Betroffen sind:
  - 15 Ressourcennamen, darunter `HPOSeverity` → `MII_VS_Seltene_HPOSeverity`, `PenetranceValueSet` → `MII_VS_Seltene_Penetrance`, `MII_LM_SE` → `MII_LM_Seltene` und `MII_CPS_SE_CapabilityStatement` → `MII_CPS_Seltene_CapabilityStatement`. Namen sind nicht Teil des Canonicals — **hier ändert sich keine URL**
  - zwei Ids: die Extension `von-seltene-betroffen` (am 02.09. zwischenzeitlich `mii-ex-von-se-betroffen`) heißt jetzt `mii-ex-seltene-von-se-betroffen`, ihr ValueSet `von-seltene-betroffen-vs` heißt `mii-vs-seltene-von-se-betroffen`. **Beide Canonicals ändern sich**
  - 86 Beispielinstanzen erhalten das Präfix `mii-exa-seltene-`, alle 281 Verweise untereinander und auf den Fallbeispielseiten mitgezogen
...
- `fix` **zwei Beispiele widersprachen sich fachlich.** Die Deletion der Exons 45-47 im DMD-Gen wurde an einer Stelle als „Out-of-frame … Duchenne-Muskeldystrophie" geführt, im Ausschlussbeispiel dagegen als „In-frame … vereinbar mit Becker, NICHT Duchenne". Die Arithmetik entscheidet: `c.6439`–`c.6912` sind 474 Basen, 474 ÷ 3 = 158 — das Leseraster bleibt erhalten. Die Deletion ist in-frame und führt zu Becker. Variante und diagnostischer Bericht sind korrigiert (`conclusionCode` jetzt OMIM 300376 Becker); das bestätigte Duchenne-Beispiel hat mit der Deletion des Exons 45 einen echten out-of-frame Beleg erhalten (176 Basen, nicht durch drei teilbar)
- `fix` **13 Referenzen liefen ins Leere.** Beispiele verwiesen auf Ressourcen, die nie existierten — darunter alle fünf Belege der Ausschlussbeispiele, obwohl gerade diese Seite den Ausschluss als dokumentationspflichtig herausstellt. Acht Verweise ließen sich auf vorhandene Beispiele umbiegen, für die übrigen sind die fehlenden Ressourcen angelegt worden: Linsenbefund und Beighton-Score als ausgeschlossene HPO-Phänotypen (`component[status] = Absent`), Laktat, Muskelbiopsie, CFTR-Anlageträgerschaft, PAH-Variante, zwei generische Kontakte und ein Beispiel für die Studieneinschluss-Anfrage, die bisher als einziges Profil ohne Beispiel dastand. Sämtliche neuen Codes gegen tx.fhir.org, die HPO-API und genenames.org geprüft — vier davon waren im ersten Anlauf falsch
