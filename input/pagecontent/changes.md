...
- `change` **the logical model's canonical changed.** It was `…/StructureDefinition/LogicalModel/Seltene` and is now `…/StructureDefinition/LogicalModel/mii-lm-seltene`. The `LogicalModel/` segment stays — it is the MII majority — and only the tail carries the id instead of the name, as the IG Publisher requires. **References to the old canonical no longer resolve.** The 17 mapping targets inside the module were carried along. For context: the old pattern is the MII majority — 15 of 23 logical models across 10 modules use `…/StructureDefinition/LogicalModel/<Name>`, `kerndatensatz-base` among them; seven modules already use the id scheme. The CapabilityStatement's canonical (`…/CapabilityStatement/metadata`) is unchanged and exempted from the check via `special-url`
...
- `fix` **two examples contradicted each other clinically.** The deletion of exons 45-47 in the DMD gene was documented once as "out-of-frame … Duchenne muscular dystrophy" and, in the exclusion example, as "in-frame … consistent with Becker, NOT Duchenne". The arithmetic settles it: `c.6439`–`c.6912` spans 474 bases, 474 ÷ 3 = 158 — the reading frame is preserved. The deletion is in-frame and causes Becker. Variant and diagnostic report corrected (`conclusionCode` now OMIM 300376 Becker); the confirmed Duchenne example received a genuine out-of-frame finding, the deletion of exon 45 (176 bases, not divisible by three)
- `fix` **13 references pointed at nothing.** Examples referenced resources that never existed — including all five pieces of evidence in the exclusion examples, on the very page that presents documenting exclusions as mandatory. Eight references were repointed to existing examples; for the rest the missing resources were created: lens finding and Beighton score as excluded HPO phenotypes (`component[status] = Absent`), lactate, muscle biopsy, CFTR carrier status, PAH variant, two generic encounters, and an example for the study-inclusion request, until now the only profile without one. Every new code checked against tx.fhir.org, the HPO API and genenames.org — four were wrong on the first attempt
