...
- `change` **the logical model's canonical changed.** It was `…/StructureDefinition/LogicalModel/Seltene` and now follows `…/StructureDefinition/mii-lm-seltene`, the `{canonical}/{type}/{id}` scheme the IG Publisher expects. **References to the old canonical no longer resolve.** The 17 mapping targets inside the module were carried along. For context: the old pattern is the MII majority — 15 of 23 logical models across 10 modules use `…/StructureDefinition/LogicalModel/<Name>`, `kerndatensatz-base` among them; seven modules already use the id scheme. The CapabilityStatement's canonical (`…/CapabilityStatement/metadata`) is unchanged and exempted from the check via `special-url`
