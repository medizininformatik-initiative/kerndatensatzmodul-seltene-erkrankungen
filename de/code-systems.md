...
# Datei Alpha-ID-SE (Mapping-Datei für Seltene Erkrankungen)
...
# MONDO (Monarch Disease Ontology) - Sekundäre Harmonisierungsontologie
...
1. **Integration mit internationalen Projekten:**MONDO ist das empfohlene Krankheitskodiersystem für:
...
1. **Semantische Interoperabilität:**MONDO ermöglicht die Verknüpfung von Daten, die mit unterschiedlichen nationalen Klassifikationen kodiert wurden
...
Neben den oben beschriebenen externen Terminologien definiert das Modul die folgenden eigenen CodeSystems. Sie bestehen dort, wo keine publizierte Terminologie das Konzept führt.
...
| MII CS Seltene Erkrankungen HPO Change Status | [`mii-cs-seltene-hpo-change-status`](CodeSystem-mii-cs-seltene-hpo-change-status.md) | CodeSystem zur Dokumentation von Änderungen bei HPO-Phänotypen über Zeit. Eigene Codes des Moduls: HPO führt für diesen Zweck nichts. Sein Zweig Clinical course (HP:0031797) beschreibt den Verlauf der Krankheit — Onset, Pace of progression, Temporal pattern, Disease remission —, nicht die Veränderung eines einzelnen Phänotyps zwischen zwei Erhebungen. Vier der fünf Konzepte haben SNOMED-CT-Entsprechungen, die je Code unten angegeben sind; für newly-added führt SNOMED kein generisches Konzept. |
