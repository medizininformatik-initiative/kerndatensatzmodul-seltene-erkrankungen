...
tar -czf ../de.medizininformatikinitiative.kerndatensatz.seltene-[VERSION].tgz package/
...
```bash
cd .bake && tar -czf ../de.medizininformatikinitiative.kerndatensatz.seltene-2026.0.1.tgz package/ && cd ..
```
...
tar -tzf de.medizininformatikinitiative.kerndatensatz.seltene-[VERSION].tgz | head -20

# Check file count
tar -tzf de.medizininformatikinitiative.kerndatensatz.seltene-[VERSION].tgz | wc -l

# Check package size
ls -lh de.medizininformatikinitiative.kerndatensatz.seltene-[VERSION].tgz
```

**Expected Results**:
- File count: 130-160 files (NOT 300+)
- Package size: 350-450 KB (NOT 2+ MB)
- Structure:
  ```
  package/package.json
  package/StructureDefinition-mii-pr-seltene-*.json
...
tar -tzf de.medizininformatikinitiative.kerndatensatz.seltene-[VERSION].tgz | grep -E '\.fsh$|\.claude|input/fsh'
...
tar -xzf de.medizininformatikinitiative.kerndatensatz.seltene-[VERSION].tgz -C /tmp/package-verify

# Check a key profile (e.g., Familienanamnese or HPO Observation)
cat /tmp/package-verify/package/StructureDefinition-mii-pr-seltene-familienanamnese.json | grep -E '"url"|"version"'
...
ls /tmp/package-verify/package/StructureDefinition-*.json | wc -l  # Should be ~18-30
ls /tmp/package-verify/package/ValueSet-*.json | wc -l             # Should be ~15-20
ls /tmp/package-verify/package/examples/*.json | wc -l             # Should be ~100+
...
**Version**: [version]
**Tarball**: de.medizininformatikinitiative.kerndatensatz.seltene-[version].tgz
**Size**: [actual size, should be ~350-450 KB]
**Total Files**: [count, should be 130-160]
**Resources**:
...
**Tarball Location**: ./de.medizininformatikinitiative.kerndatensatz.seltene-[version].tgz

✓ Package structure verified
✓ Critical resources present (Familienanamnese, HPO Observation, etc.)
