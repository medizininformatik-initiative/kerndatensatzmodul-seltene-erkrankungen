...
5. **Attach FHIR package to release**
   ```bash
   gh release upload v{VERSION} de.medizininformatikinitiative.kerndatensatz.seltene-{VERSION}.tgz
   ```

   Or if the package is in `.bake/`:
   ```bash
   gh release upload v{VERSION} .bake/de.medizininformatikinitiative.kerndatensatz.seltene-{VERSION}.tgz
   ```
